...
class Game(arcade.Window):
    ...
    def check_collisions(self):
        ...
        for laser in self.lasers:
            for asteroid in self.asteroids:
                ...
                if laser.alive and asteroid.alive:
                    ...
                    if ((laser.center.x - asteroid.center.x)**2 + (laser.center.y - asteroid.center.y)**2)**.5 < too_close:
                        # its a hit!
                        laser.alive = False
                        list = asteroid.split()
                        self.asteroids += list 
        ...
        for asteroid in self.asteroids:
            ...
            if self.ship.alive and asteroid.alive:
                ...
                if ((self.ship.center.x - asteroid.center.x)**2 + (self.ship.center.y - asteroid.center.y)**2)**.5 < too_close:
                    # its a hit!
                    if self.frame > 40:
        ...
        for laser in self.lasers:
            ...
            if self.ship.alive and laser.alive and laser.time_alive > 30:
                ...
                if ((self.ship.center.x - laser.center.x)**2 + (self.ship.center.y - laser.center.y)**2)**.5 < too_close:
                    # its a hit!
                    laser.alive = False
    ...
    def draw_health(self):
        ...
        arcade.draw_text("Hull Integrity:", start_x = 215, start_y = SCREEN_HEIGHT - 15, font_size=12, color=arcade.color.WHITE)
